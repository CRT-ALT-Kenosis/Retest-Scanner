...
def run_single_backtest(ticker, as_of_date, w_dist_200sma, w_prior_run, w_correction,
            ...
            def render_category(label, emoji, cat_class, group):
                ...
                for h in group:
                    ...
                    <div class="hit-card">
                        ...
                        <div style="margin-top:0.5rem;font-size:0.62rem;color:#475569;font-family:Space Mono;line-height:1.8">
                            200W SMA: <b style="color:#94a3b8">${wr.get('sma200','—')}</b> &nbsp;|&nbsp;
                            Dist from 200W: <b style="color:#94a3b8">{wr.get('dist_200sma_pct','—')}%</b> &nbsp;|&nbsp;
                            Prior Run: <b style="color:#94a3b8">{wr.get('prior_run_pct','—')}%</b> &nbsp;|&nbsp;
                            ATH Corr: <b style="color:#94a3b8">{wr.get('correction_from_ath_pct','—')}%</b>
                            <br>
                            Vol Ratio: <b style="color:#94a3b8">{wr.get('vol_ratio','—')}×</b> &nbsp;|&nbsp;
                            Daily ATR%: <b style="color:#94a3b8">{dr.get('atr_pct','—')}%</b> &nbsp;|&nbsp;
                            50D dist: <b style="color:#94a3b8">{dr.get('pct_above_50sma','—')}%</b> &nbsp;|&nbsp;
                            EMA10 vs EMA20: <b style="color:#94a3b8">{dr.get('ema10_vs_ema20_pct','—')}%</b>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
...
with tab3:
    ...
    bt_tab_a, bt_tab_b = st.tabs(["📋  Known Setup Validator", "📅  Date Picker Scan"])
            ...
            for s in to_run:
                ...
                with st.spinner(f"Testing {ticker} as of {s['date']}..."):
                    result = run_single_backtest(
                        ticker, as_of,
                        w_dist_200sma, w_prior_run, w_correction, w_vol_mult,
