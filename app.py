...
with st.sidebar:
    ...
    if is_retest:
        ...
        w_prior_run  = st.slider("Min prior run from base (%)", 100, 2000, 500)
        w_correction = st.slider("Min correction from ATH (%)", 30, 85, 50)
        w_vol_mult   = st.slider("Weekly volume surge multiplier", 1.0, 5.0, 1.5)
...
def check_weekly(df_w, w_dist_200sma_lo, w_dist_200sma_hi, w_prior_run, w_correction, w_vol_mult):
    ...
    # ── ATH: rolling 4yr window ending 8 weeks ago ────────────────────────────
    # Excludes last 8 weeks so a current breakout isn't counted as the ATH.
    # For TPL: finds the ~$270 2022 peak, not the 2025 run.
    # For LITE: finds the $391 2021 ATH correctly.
    # For KGC: finds the ~$10 2020 ATH, giving a real prior run figure.
    win_end   = max(0, len(closes) - 8)
    win_start = max(0, win_end - 208)           # 208w = 4 years
    ath_w     = closes.iloc[win_start:win_end]
    ath       = ath_w.max() if len(ath_w) > 0 else closes.iloc[:win_end].max()
    atl       = closes.iloc[:win_end].min() if win_end > 0 else closes.min()
...
def score_setup(wr, dr):
    ...
    # Correction — proportional 0–20pts (30% = 0pts, 80% = 20pts)
    corr = wr.get("correction_from_ath_pct", 0)
    pts += min(20, max(0, (corr - 30) / 50 * 20))
            ...
            def render_category(label, emoji, cat_class, group):
                ...
                for h in group:
                    ...
                    html = (
                        ...
                        f'<span class="score-chip">{score}/110</span>' +
                        f'</div></div>' +
                        f'<div style="margin-top:0.35rem">{sec_badge}</div>' +
                        f'<div style="margin-top:0.5rem">{badges}</div>' +
                        f'<div class="metric-strip" style="margin-top:0.4rem">{detail}</div>' +
                        f'</div>'
                    )
