...
@media (max-width: 768px) {{
    .stTabs [data-baseweb="tab-list"] {{
        ...
        /* Sit above iOS Safari browser chrome (≈83px) + home indicator */
        bottom: 0 !important;
        left: 0 !important;
        right: 0 !important;
        ...
        /* Top padding for tabs, bottom padding accounts for iOS home indicator */
        ...
        /* Push tab bar ABOVE browser chrome using transform */
    ...
    /* Content area: pad enough for tab bar (≈56px) + iOS safe area + browser chrome buffer */
    .main .block-container {{
        padding-bottom: calc(9rem + env(safe-area-inset-bottom, 34px)) !important;
...
def sector_heat_strip(sector_returns, hit_sectors=None):
    ...
    for etf, rel in sorted_items:
        ...
        rows_html.append(
            f'<div class="sector-row" title="{full}: {sign}{rel}% vs SPY (26W)">'
            f'  <div class="sector-row-label">{short}</div>'
            f'  <div class="sector-row-bar-wrap">'
            f'    <div class="sector-row-bar" style="width:{bar_w}%;background:{color};'
            f'         opacity:0.85"></div>'
            f'  </div>'
            f'  <div class="sector-row-val" style="color:{text_color}">{sign}{rel}%</div>'
            f'  <div class="sector-row-hits" style="color:{color}">{hits_txt}</div>'
            f'</div>'
        )

    joined = "\n".join(rows_html)
    return (
        f'<div class="sector-heat">'
        f'  <div class="sector-heat-title">Sector momentum · 26W vs SPY</div>'
        f'  {joined}'
        f'</div>'
    )
...
with tab2:
        ...
        for i, ticker in enumerate(scan_universe):
            ...
            if sc >= min_display:
                ...
                _save_state(hits, logs, i + 1, skipped, total,
                            ...
                            scan_ts)
                # ── Gist checkpoint every 10 hits — survives container restart ──
                if len(hits) % 10 == 0:
    ...
    # ── Watchlist panel — always rendered ABOVE results (not in elif) ───────────
    # BUG FIX: was if/elif which meant starring suppressed results.
    # Now always-visible compact strip + expandable full panel.
    watchlist_data = st.session_state.get("watchlist", {})
    if watchlist_data:
        _gs2_c = "#10b981" if _gist_enabled() else "#64748b"
        _gs2   = "● Gist sync" if _gist_enabled() else "○ No sync"
        _wl_tickers = list(watchlist_data.keys())
        # ── Compact always-visible ticker strip ──────────────────────────────
        _wl_pills = " ".join(
            ...
            for t in _wl_tickers[:10]
        )
        _wl_more = (f' <span style="color:var(--text-muted);font-size:0.6rem">+{len(_wl_tickers)-10} more</span>'
                    if len(_wl_tickers) > 10 else "")
        st.markdown(
            f'<div style="background:var(--bg-card);border:1px solid var(--border);'
            f'border-radius:12px;padding:0.55rem 1rem;margin:0.6rem 0;'
            f'display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap">'
            f'<span style="font-family:DM Mono,monospace;font-size:0.58rem;'
            f'color:{_gs2_c};font-weight:700;white-space:nowrap">⭐ {_gs2}</span>'
        ...
        with st.expander(
            f"⭐ Manage Watchlist — {len(watchlist_data)} stock{'s' if len(watchlist_data) != 1 else ''}",
            expanded=False
        ):
...
3. Save → app restarts → star any ticker → Gist ID appears below → paste into secrets as `gist_id`
            ...
            with wl_cols[0]:
                ...
                for tk, d in watchlist_data.items():
                    ...
                    _wl_c1, _wl_c2 = st.columns([5, 1])
                    _wl_c1.markdown(
                        f'<div style="display:inline-flex;align-items:center;gap:0.6rem;'
                        f'background:var(--bg-raised);border:1px solid var(--border);'
                        f'border-radius:8px;padding:0.35rem 0.8rem;margin:0.15rem 0">'
                        f'<b style="font-family:DM Mono,monospace;color:var(--accent)">{tk}</b>'
                        f'<span style="font-family:DM Mono,monospace;font-size:0.62rem;'
                        f'color:var(--text-muted)">${d["close"]} · {score_norm}/100 · '
                        f'{d["structure"] or d["slope"] or d["sector"]}</span>'
                        f'<a href="{tv_link_wl}" target="_blank" style="font-size:0.65rem">📈</a>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                    if _wl_c2.button("✕", key=f"rm_{tk}", help=f"Remove {tk}"):
