...
def fetch_tradingview_tickers(
    ...
    Filters: market cap tiers (union of selected ranges) + min daily share volume.
...
def check_weekly(df_w, w_dist_200sma_lo, w_dist_200sma_hi, w_prior_run, w_correction, w_vol_mult):
    ...
    res.update({
        ...
        "pass_sma200_slope":      slope_grade != "declining",  # rising or flattening passes
    })
...
with tab2:
    ...
    if run_scan:
        ...
        if not hits:
            ...
            watchlist    = [h for h in hits_sorted if h["norm_score"] < 60]
            ...
            def render_category(label, emoji, cat_class, group):
                ...
                for h in group:
                    ...
                    if st.button(_blabel, key=f"wl_{ticker}_{score}", help="Add/remove from watchlist"):
                        ...
                        # Persist watchlist change to /tmp immediately
                        try:
                            if os.path.exists(SAVE_PATH):
