...
gist_id = ""
```
5. Save secrets → app restarts → **⭐ star any ticker** → the app auto-creates your Gist
6. Check the watchlist panel — a `Gist ID: xxxx` line will appear · paste it into secrets as `gist_id` → save again
                """)

        # ── Show Gist ID if user needs it for secrets config ────────────────
        ...
        if _gist_cache_id and not _secrets_gist:
            st.info(
                f"✅ Gist auto-created — **paste this ID into secrets** as `gist_id`:  \n\n"
                f"```\n{_gist_cache_id}\n```\n\n"
                f"→ **Manage App → Secrets** → add `gist_id = \"{_gist_cache_id}\"` → save",
                icon="🔗"
            )
        ...
        with wl_cols[0]:
            ...
            for tk, d in watchlist_data.items():
                ...
                st.markdown(
                    ...
                    f'{d["structure"]} · {d["slope"]} · {d["sector"]}</span>'
                    f'<a href="{tv_link_wl}" target="_blank" style="font-size:0.65rem">📈</a>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                if st.button("✕", key=f"rm_{tk}", help=f"Remove {tk} from watchlist"):
