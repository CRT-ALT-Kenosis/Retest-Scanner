...
with st.sidebar:
    ...
    if is_retest:
        ...
        w_prior_run  = st.slider("Min prior run from base (%)", 50, 1000, 200)
        w_correction = st.slider("Min correction from ATH (%)", 20, 80, 35)
        w_vol_mult   = st.slider("Weekly volume surge multiplier", 1.0, 5.0, 1.3)
    ...
    min_display = st.slider("Min score to display", 0, 80, 40,
