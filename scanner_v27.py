...
def gist_save_watchlist(watchlist_dict):
    ...
    On first creation, stores the new Gist ID in:
      st.session_state["_gist_id_cache"]   ← used for all saves this session
      st.session_state["_gist_id_new"]     ← displayed prominently in UI until
                                              user pastes it into secrets
    ...
    try:
        ...
        if gist_id:
            ...
            if r.status_code == 201:
                ...
                st.session_state["_gist_id_new"]   = new_id
                return True
...
.sector-dot:hover .sector-dot-circle { transform: scale(1.2); }
.sector-dot-label {
    font-family: DM Mono, monospace; font-size: 0.5rem;
    color: var(--text-muted); text-align: center; max-width: 36px;
...
@media (max-width: 768px) {{
    .stTabs [data-baseweb="tab-list"] {{
        ...
        right: 0 !important; z-index: 999 !important;
        border-radius: 0 !important; border-top: 1px solid var(--border) !important;
        border-left: none !important; border-right: none !important;
        border-bottom: none !important;
        padding: 0.3rem 0.5rem 1.2rem !important;
...
def sector_heat_strip(sector_returns, hit_sectors=None):
    """
    Visual sector heat strip — Principle 05.
    Dots sized by number of hits in sector, colored by 26W momentum.
    hit_sectors: dict of {sector_name: hit_count}
    """
    ...
    etf_labels = {
        ...
        "XLU":"Utils","XLP":"Staples","XLC":"Comms",
    }
    hit_sectors = hit_sectors or {}
    dots = []
    for etf, rel in sorted(sector_returns.items(), key=lambda x: -x[1])[:11]:
        label = etf_labels.get(etf, etf)
        # Color by momentum
        if rel >= 10:
            color = "#10b981"
        elif rel >= 3:
            ...
            color = "#f43f5e"
        # Size by hits (8px base, +3px per hit, max 22px)
        hits = hit_sectors.get(label, 0)
        size = min(22, 8 + hits * 3)
        sign = "+" if rel >= 0 else ""
        dot = (
            f'<div class="sector-dot" title="{label}: {sign}{rel}% &middot; {hits} hits">'
            f'<div class="sector-dot-circle" style="width:{size}px;height:{size}px;'
            f'background:{color};box-shadow:0 0 {size//2}px {color}55"></div>'
            f'<span class="sector-dot-label">{label}</span>'
            f'</div>'
        )
        dots.append(dot)
    joined = "".join(dots)
    return f'<div class="sector-heat">{joined}</div>'
...
with tab2:
    ...
    if _wl_data:
        _gs = ('● Gist sync on' if _gist_enabled() else '○ Gist not configured')
        _gs_c = '#10b981' if _gist_enabled() else '#64748b'
        st.markdown(
            f'<div class="section-header">⭐ Watchlist '
            f'<span style="font-family:DM Mono,monospace;font-size:0.6rem;color:{_gs_c}">{_gs}</span>'
            f'</div>', unsafe_allow_html=True)

        # ── Gist ID banner: shown when gist was just auto-created ──────────
        _new_gist_id = st.session_state.get("_gist_id_new", "")
        _secrets_gist_id = ""
        try:
            _secrets_gist_id = st.secrets["gist"].get("gist_id", "").strip()
        except Exception:
            pass
        if _new_gist_id and not _secrets_gist_id:
            st.warning(
                f"**📋 Action needed — paste this Gist ID into your Streamlit secrets:**\n\n"
                f"`{_new_gist_id}`\n\n"
                f"Go to **Manage App → Secrets** and set `gist_id = \"{_new_gist_id}\"`  \n"
                f"Until you do, each app restart creates a new Gist and your watchlist resets.",
                icon="⚠️"
            )
            if st.button("✅ I've pasted it into secrets", key="gist_id_ack"):
                st.session_state.pop("_gist_id_new", None)
                st.rerun()
        # ── Gist error display ──────────────────────────────────────────────
        _gist_err = st.session_state.pop("_gist_last_error", None)
        if _gist_err:
            st.error(f"Gist save failed: {_gist_err}", icon="❌")
        _wl_hdr = st.columns([2,1,1,2,2,1])
        for _lbl, _col in zip(["Ticker","Score","Price","Sector","Structure",""],_wl_hdr):
            _col.markdown(f'<span style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--text-muted)">{_lbl}</span>', unsafe_allow_html=True)
        _wl_export = []
        for _t, _d in sorted(_wl_data.items(), key=lambda x: -x[1]["score"]):
            _rc = st.columns([2,1,1,2,2,1])
            _rc[0].markdown(f'<b style="font-family:DM Mono,monospace">{_t}</b>', unsafe_allow_html=True)
            _rc[1].markdown(f'<span style="font-family:DM Mono,monospace;font-size:0.8rem;color:var(--amber)">{_d["score"]}/100</span>', unsafe_allow_html=True)
            _rc[2].markdown(f'<span style="font-family:DM Mono,monospace;font-size:0.75rem">${_d["close"]}</span>', unsafe_allow_html=True)
            _rc[3].markdown(f'<span style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--text-muted)">{_d["sector"]}</span>', unsafe_allow_html=True)
            _rc[4].markdown(f'<span style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--green)">{_d["structure"]}</span>', unsafe_allow_html=True)
            if _rc[5].button("✕", key=f"rm_{_t}", help=f"Remove {_t}"):
                del st.session_state["watchlist"][_t]
                gist_save_watchlist(st.session_state["watchlist"])
                st.rerun()
            _wl_export.append({"Ticker":_t,"Score":_d["score"],"Close":_d["close"],
                               "Sector":_d["sector"],"Structure":_d["structure"],
                               "Slope":_d.get("slope",""),"Added":_d["added"]})
        _wl_df = pd.DataFrame(_wl_export)
        _wl_c1, _wl_c2 = st.columns([1,4])
        _wl_c1.download_button("⬇ Export Watchlist", _wl_df.to_csv(index=False),
                               "watchlist.csv", "text/csv")
        if _wl_c2.button("🗑 Clear Watchlist"):
            st.session_state["watchlist"] = {}
            gist_save_watchlist({})
            st.rerun()

    # ── Watchlist panel (always visible when items exist) ────────────────────
    watchlist_data = st.session_state.get("watchlist", {})
    if watchlist_data:
        _gs2 = ('● Gist sync on' if _gist_enabled() else '○ Gist not configured')
        _gs2_c = '#10b981' if _gist_enabled() else '#64748b'
        st.markdown(
            f'<div class="section-header">⭐ Watchlist '
            f'<span style="font-family:DM Mono,monospace;font-size:0.6rem;color:{_gs2_c}">{_gs2}</span>'
            f'</div>', unsafe_allow_html=True)
        ...
        if not _gist_enabled():
            with st.expander("🔗 Enable cross-device sync — survives container restarts"):
                st.markdown("""
**3-minute setup — watchlist persists on iPhone, desktop, anywhere:**

1. Go to [github.com/settings/tokens](https://github.com/settings/tokens) → **Generate new token (classic)**
2. Name it `scanner-watchlist`, tick **gist** scope only → Generate
3. Copy the token
4. In Streamlit Cloud → **Manage App** → **Secrets**, add:
```toml
[gist]
token   = "ghp_yourtoken"
gist_id = ""
```
5. Save secrets → app restarts → star any ticker → a yellow banner appears with your Gist ID
6. Copy the Gist ID from the banner → paste into secrets as `gist_id` → save again
                """)
        elif not st.session_state.get("_gist_id_cache"):
            # Token set but gist_id blank in secrets and not yet created this session
            try:
                _sid = st.secrets["gist"].get("gist_id", "").strip()
            except Exception:
                _sid = ""
            if not _sid and not st.session_state.get("_gist_id_new"):
                st.info(
                    "✅ Gist token active — **star any ticker** to auto-create your Gist. "
                    "A banner will appear with the Gist ID to paste into secrets.",
                    icon="🔗"
                )
        ...
        with wl_cols[0]:
            wl_rows = []
            for tk, d in watchlist_data.items():
                tv_link_wl = f"https://www.tradingview.com/chart/?symbol={tk}&interval=W"
                score_norm = min(100, round(d["score"] / 1.25))
                st.markdown(
                    f'<div style="display:inline-flex;align-items:center;gap:0.6rem;'
                    f'background:var(--bg-card);border:1px solid var(--border);'
                    f'border-radius:8px;padding:0.4rem 0.8rem;margin:0.2rem">'
                    f'<span style="font-family:DM Mono,monospace;font-weight:700;'
                    f'color:var(--accent)">{tk}</span>'
                    f'<span style="font-family:DM Mono,monospace;font-size:0.65rem;'
                    f'color:var(--text-muted)">${d["close"]} · {score_norm}/100 · '
                    f'{d["structure"]} · {d["slope"]}</span>'
                    f'<a href="{tv_link_wl}" target="_blank" style="font-size:0.65rem">📈</a>'
                    f'</div>',
                    unsafe_allow_html=True
                )
                wl_rows.append({
                    ...
                    "Sector":    d["sector"],
                    "Added":     d["added"],
                    "TradingView": f"https://www.tradingview.com/chart/?symbol={tk}&interval=W",
                })
        with wl_cols[1]:
            if wl_rows:
        ...
        def render_category(label, emoji, cat_class, group):
            ...
            for h in group:
                ...
                card_class = "hit-card-full" if h["score"] >= 80 else ("hit-card-strong" if h["score"] >= 60 else "hit-card-watch")
                st.markdown(
                    f'<div class="hit-card {card_class}" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:0.4rem">' +
                    f'<span class="ticker-label">{ticker}</span>' +
                    f'<span style="font-family:DM Mono,monospace;font-size:0.65rem;color:var(--text-muted)">{wr.get("dist_200sma_pct",0):+.1f}% · Corr {wr.get("correction_from_ath_pct",0):.0f}% · Vol ×{wr.get("vol_ratio",0):.1f}</span>' +
                    f'<div style="display:flex;align-items:center;gap:0.75rem">' +
                    f'<span class="price-label">${price}</span>' +
                    f'<span class="score-chip">{score}/100</span>' +
